...
**받은 뒤에는 MCP 서버를 다시 시작해야 반영됩니다.** 서버는 기동할 때 DB를 한 번
열고 그 핸들을 계속 쓰는데, `update-data`는 파일을 통째로 바꿔치기하므로 이미 떠
있는 서버는 교체 전 파일을 계속 읽습니다. Claude Desktop·Cursor처럼 프로세스를
직접 띄우는 클라이언트는 클라이언트를 재시작하면 됩니다. 갱신됐는지는
`get_data_status`의 수집 시각으로 확인하세요.

### 구판 처리

원본은 규정이 개정되면 새 `bookid`를 발급하고 옛 `bookid`를 목록에서 빼지만,
삭제 신호는 주지 않습니다. 수집기는 이번 목록에 없던 행을 구판으로 표시하고
(`missing_since`), 검색·목록·최신 개정 조회에서 기본 제외합니다. 지우지는 않으므로
`bookid`로 직접 조회하면 구판 전문도 볼 수 있고, 이때는 구판이라는 경고가 함께
나옵니다. 구판을 목록에 포함하려면 `list_regulations`에 `include_superseded`를
지정하세요. 현재 구판 건수는 `get_data_status`에 표시됩니다.

### 규정 종류 보정

원본의 `bookcd`(정관·규정·시행세칙·내규)에는 오분류가 있습니다. "ESG연구소 규정"과
"마이크로전공에 관한 시행세칙"이 정관으로, "상명대학교산학협력단 정관"이 규정으로
등록돼 있는 식으로 현행 301건 중 9건이 어긋납니다. 규칙성이 없어 원본 등록 시의
입력 오류로 보입니다.

원본 값은 `bookcd`에 그대로 두고, 제목 접미사로 보정한 값을 `bookcd_norm`에 따로
둡니다. `type` 필터와 화면의 "구분" 표시는 보정값을 씁니다. 판정 근거는 제목
접미사뿐이라 "학칙"·"학생 준칙"·"동문회 회칙"처럼 네 종류에 없는 접미사면 원본 값을
그대로 씁니다(이 경우 원본의 오분류도 그대로 남습니다).
